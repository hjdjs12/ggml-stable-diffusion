...
void update_matmul_addr(uint64_t *ops, uint32_t input, uint32_t weights, uint32_t output) {
    ops[21] = NPUOP(OP_REG_CNA, input, CNA_FEATURE_DATA_ADDR);
    ops[30] = NPUOP(OP_REG_CNA, weights, CNA_DCOMP_ADDR0);
    ops[57] = NPUOP(OP_REG_DPU, output, DPU_DST_BASE_ADD);
}
/*
...
void gen_matmul_task(uint64_t *ops, npu_cna_desc *cna_desc, npu_core_desc *core_desc, npu_dpu_desc *dpu_desc) {
    ...
    value =
        ((cna_desc->proc_precision & 0x7) << 7) | ((cna_desc->in_precision & 0x7) << 4) | (cna_desc->conv_mode & 0xf);
    ...
    value = ((cna_desc->weight_width & 0x1F) << 24) | ((cna_desc->weight_height & 0x1F) << 16) |
            (cna_desc->weight_kernels & 0x3FFF); // N dim only support 8192
    ...
    value = ((dpu_desc->size_e_2 & 0x7) << 8) | ((dpu_desc->size_e_1 & 0x7) << 5) | ((dpu_desc->size_e_0 & 0x7) << 2) |
            ((dpu_desc->od_bypass & 0x1) << 1);
...
int gen_matmul_fp16(matmul_params_t *params) {
    ...
    cna_desc.weight_bytes_per_kernel =
        cna_desc.weight_width * cna_desc.weight_height * cna_desc.datain_channel * sizeof(__fp16);
    ...
    cna_desc.data_entries = (((cna_desc.datain_width * cna_desc.datain_channel) % 32) == 0) ? cna_desc.data_entries
...
int gen_matmul_int8(matmul_params_t *params) {
    ...
    cna_desc.weight_kernels = params->n;
    cna_desc.weight_bytes_per_kernel =
        cna_desc.weight_width * cna_desc.weight_height * cna_desc.datain_channel * sizeof(int8_t);
    cna_desc.weight_bytes = cna_desc.weight_bytes_per_kernel * cna_desc.weight_kernels;
    ...
    cna_desc.data_entries = (((cna_desc.datain_width * cna_desc.datain_channel) % 64) == 0) ? cna_desc.data_entries
                                                                                            : cna_desc.data_entries + 1;
    cna_desc.data_sign = 0x1;
    cna_desc.cvt_type = 0x1;
    cna_desc.cvt_bypass = 0x1;
